...
export async function POST(req: NextRequest) {
  ...
  const { czechReply: rawReply } = await req.json();
  if (!rawReply?.trim()) {
    return NextResponse.json({ error: "Text odpovědi je prázdný." }, { status: 400 });
  }
  // Strip **bold** markers before translation so the Hungarian email stays clean
  ...
  console.log(`[translate-reply] DeepL CS→HU | free key: ${isFreeKey} | endpoint: ${endpoint}`);
